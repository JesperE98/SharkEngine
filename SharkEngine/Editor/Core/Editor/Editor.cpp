...
Editor::Editor()
{
	SHARK_LOG(Editor, "Editor::Edtior() - Created Editor.");
...
void Editor::CheckGLErrors(const std::string& context)
{
	GLenum err;
	while ((err = glGetError()) != GL_NO_ERROR) {
		SHARK_ERR(OpenGL, "Editor::CheckGlErrors() - {}: OpenGL error: {}", context, err);
	}
...
void Editor::CreateEditorWindow()
{
	...
	SHARK_LOG(Editor, "Editor::CreateEditorWindow() - Creating Editor Window.");
...
void Editor::RenderPanels(float deltaTime)
{
	GLenum err;
	while ((err = glGetError()) != GL_NO_ERROR) {
		SHARK_ERR(OpenGL, "Editor::RenderPanels() - OpenGL error before rendering panels: {}", err);
	}
	...
	while ((err = glGetError()) != GL_NO_ERROR) {
		SHARK_ERR(OpenGL, "Editor::RenderPanels() - OpenGL error after rendering panels: {}", err);
	}
}
...
void Editor::Render(){
	...
	GLenum err;
	while ((err = glGetError()) != GL_NO_ERROR) {
		SHARK_ERR(OpenGL, "Editor::Render() - OpenGL error after ImGui: {}", err);
	}
